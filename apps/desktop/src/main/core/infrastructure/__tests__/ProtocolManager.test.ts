...
const { mockApp, mockGetProtocolScheme, mockParseProtocolUrl } = vi.hoisted(() => ({
  ...
  mockGetProtocolScheme: vi.fn().mockReturnValue('agentasia'),
...
describe('ProtocolManager', () => {
  ...
  beforeEach(() => {
    ...
    mockGetProtocolScheme.mockReturnValue('agentasia');
  ...
  describe('constructor', () => {
    it('should initialize with protocol scheme from getProtocolScheme', () => {
      ...
      expect(manager.getScheme()).toBe('agentasia');
    });
  });
  ...
  describe('initialize', () => {
    it('should register protocol handlers', () => {
      ...
      expect(app.setAsDefaultProtocolClient).toHaveBeenCalledWith('agentasia');
    });
  ...
  describe('protocol registration', () => {
    ...
    it('should verify registration status after registering', () => {
      ...
      expect(app.isDefaultProtocolClient).toHaveBeenCalledWith('agentasia');
    });
  });
  ...
  describe('getProtocolUrlFromArgs', () => {
    ...
    it('should extract protocol URL from command line arguments', () => {
      ...
      const result = manager['getProtocolUrlFromArgs']([
        '/path/to/app',
        'agentasia://plugin/install?url=https://example.com',
      ]);

      expect(result).toBe('agentasia://plugin/install?url=https://example.com');
    });
    ...
    it('should return first matching URL when multiple exist', () => {
      const result = manager['getProtocolUrlFromArgs']([
        'agentasia://first/action',
        'agentasia://second/action',
      ]);

      expect(result).toBe('agentasia://first/action');
    });
  });
  ...
  describe('handleProtocolUrl', () => {
    ...
    it('should store URL when app is not ready', () => {
      ...
      manager['handleProtocolUrl']('agentasia://plugin/install');

      expect(manager['pendingUrls']).toContain('agentasia://plugin/install');
  ...
  describe('event listeners', () => {
    ...
    it('should handle open-url event', async () => {
      ...
      openUrlHandler!(mockEvent, 'agentasia://plugin/install');
    ...
    it('should handle second-instance event with protocol URL', async () => {
      ...
      const mockEvent = {};
      secondInstanceHandler!(mockEvent, ['/path/to/app', 'agentasia://plugin/install']);
  ...
  describe('processPendingUrls', () => {
    ...
    it('should process all pending URLs', async () => {
      // Add pending URLs
      manager['pendingUrls'] = ['agentasia://action1', 'agentasia://action2'];
    ...
    it('should clear pending URLs after processing', async () => {
      manager['pendingUrls'] = ['agentasia://action1'];
  ...
  describe('processProtocolUrl', () => {
    ...
    it('should show main window and dispatch to handler', async () => {
      vi.mocked(parseProtocolUrl).mockReturnValue({
        action: 'install',
        originalUrl: 'agentasia://plugin/install?url=https://example.com',
        params: { url: 'https://example.com' },
        urlType: 'plugin',
      });

      await manager['processProtocolUrl']('agentasia://plugin/install');
    ...
    it('should warn and return when parseProtocolUrl returns null', async () => {
      ...
      await manager['processProtocolUrl']('agentasia://invalid');
    ...
    it('should handle errors gracefully', async () => {
      ...
      // Should not throw
      await expect(
        manager['processProtocolUrl']('agentasia://plugin/install'),
