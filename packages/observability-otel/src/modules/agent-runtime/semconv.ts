 ...
 * instrumentation, alongside AgentAsia-specific (`agentasia.*`) extensions.
 *
...
/** Provider name — `openai`, `anthropic`, `agentasia`, etc. */
...
// ---- agentasia.* (AgentAsia-specific extensions) ----

/** Internal operation id assigned by the Agent Runtime. */
export const ATTR_LOBEHUB_AGENT_OPERATION_ID = 'agentasia.agent.operation.id' as const;

/** Total step count for the agent invocation. */
export const ATTR_LOBEHUB_AGENT_STEP_COUNT = 'agentasia.agent.step.count' as const;

/** Current step index within the agent invocation. */
export const ATTR_LOBEHUB_AGENT_STEP_INDEX = 'agentasia.agent.step.index' as const;

/** Completion reason: `done` / `error` / `max_steps` / `cost_limit` / `interrupted` / `waiting_for_human`. */
export const ATTR_LOBEHUB_AGENT_COMPLETION_REASON = 'agentasia.agent.completion_reason' as const;

/** Tool execution success flag (gen_ai spec uses error.type for failures only). */
export const ATTR_LOBEHUB_TOOL_SUCCESS = 'agentasia.tool.success' as const;

/** Attempts taken to execute a tool (1 for first-try success). */
export const ATTR_LOBEHUB_TOOL_ATTEMPTS = 'agentasia.tool.attempts' as const;

/** Internal AgentAsia tool source (`builtin` / `client` / `mcp` / `composio` / `agentasiaSkill`). */
export const ATTR_LOBEHUB_TOOL_SOURCE = 'agentasia.tool.source' as const;

/** Context engineering metadata. */
export const ATTR_LOBEHUB_CONTEXT_MESSAGE_COUNT = 'agentasia.context.message_count' as const;
export const ATTR_LOBEHUB_CONTEXT_TOKEN_USAGE = 'agentasia.context.token_usage' as const;
export const ATTR_LOBEHUB_CONTEXT_WINDOW_RATIO = 'agentasia.context.window_ratio' as const;
export const ATTR_LOBEHUB_CONTEXT_KNOWLEDGE_INJECTED =
  'agentasia.context.knowledge_injected' as const;
export const ATTR_LOBEHUB_CONTEXT_KNOWLEDGE_COUNT = 'agentasia.context.knowledge_count' as const;
export const ATTR_LOBEHUB_CONTEXT_HISTORY_COMPRESSED =
  'agentasia.context.history_compressed' as const;
export const ATTR_LOBEHUB_CONTEXT_MEMORY_INJECTED = 'agentasia.context.memory_injected' as const;
export const ATTR_LOBEHUB_CONTEXT_SYSTEM_ROLE_LENGTH =
  'agentasia.context.system_role_length' as const;
export const ATTR_LOBEHUB_CONTEXT_TOOL_COUNT = 'agentasia.context.tool_count' as const;
export const ATTR_LOBEHUB_CONTEXT_HAS_IMAGES = 'agentasia.context.has_images' as const;
