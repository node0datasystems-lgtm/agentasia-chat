...
export class FileUploadService extends BaseService {
  ...
  private async assertOwnedKnowledgeBase(
    ...
    if (!permissionResult.isPermitted) {
      throw this.createAuthorizationError(permissionResult.message || 'No permission to access knowledge base files');
    }
  ...
  async getFileDetail(fileId: string): Promise<FileDetailResponse> {
    try {
      ...
      this.handleServiceError(error, 'get file detail');
    }
  }
  ...
  async handleQueries(request: BatchGetFilesRequest): Promise<BatchGetFilesResponse> {
    try {
      ...
      this.handleServiceError(error, 'batch get file details and content');
    }
  }
  ...
  private async findExistingUserFile(hash: string): Promise<FileItem | null> {
    try {
      ...
      this.handleServiceError(error, 'find existing user file by hash');
    }
  }
