...
const convertSchema = (schema: any): any => {
 ...
 // @see https://linear.app/lobehub/issue/    if (schema.enum && schema.enum.length > 0 && isStringType(schema.type)) {
 ...
 // @see https://linear.app/lobehub/issue/    if (schema.required && schema.required.length > 0 && isObjectType(schema.type)) {
